...
void check_buttons(void)
{
	if (PeripheralEn.bit.Buttons)
	{
		...
		{
			if (data_in.bit.bright == 0)						// Has Button-(right) been pressed?
			{
				...
				{
					if((*menu_ptr).m_ID == 0x0021) 				// SETUP Menu (do you wish to turn system on or off?)
					{
						if(PeripheralEn.bit.System)				// TURN OFF
						{
...
{
		lcd_clear();
		if ( !(State.bit.Fault) )	data_out.bit.led_update		= ~(data_out.bit.led_update);
        ...
        lcd_position(0x0,0x1);      	// Start Cursor @ 1st character 2nd line
