//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\\
//<<	  FILE: Interrupt_SR.c					DATE: 03/2013			   >>\\
...
__interrupt void ADCINT1_ISR(void)   // ADC  (Can also be ISR for INT10.1 when enabled)
{
	...
	AdcSignal.V_HVDC= dclinevoltagetest; //0.8*(0.1684542812*AdcResult.ADCRESULT0) + 0.2*(AdcSignal.V_HVDC)	;	// Read in the DC bridge voltage; (202.88...)*(3.4/4095)=0.1684542812
	flag.bit.V_HVDC	= 1;
...
__interrupt void EPWM7_INT_ISR(void)    // EPWM-7
{
	...
	flag.bit.DataIN = 1;					// set flags to shift data in when looping in main
	flag.bit.DataOUT = 1;					// set flags to shift data out when looping in main
